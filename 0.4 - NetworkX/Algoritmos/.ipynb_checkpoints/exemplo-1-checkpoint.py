...
## Clear
# There are no complaints  when adding existing nodes or edges. For example, after removing all nodes
# and edges,
G.clear()
##Adding new nodes/edges adn NetworkX quietly quietly ignores any that are already present.


# We add new nodes/edges and NetworkX quietly ignores any taht are already present.
G.add_edges_from([(1,2), (1, 3)])
G.add_node(1)
G.add_edge(1, 2)
...
#At thsi stage the graph G consists of 8 nodes and 3 edges, as can be seen by:
G.number_of_nodes()
G.number_of_edges()
